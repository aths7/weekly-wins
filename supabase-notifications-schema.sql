-- In-App Notification System Schema
-- Run this after the organization schema to add notification features
...
CREATE POLICY "Org admins can view invitations for their organizations" 
  ON organization_invitations 
  FOR SELECT 
  USING (
    organization_id IN (
...
CREATE POLICY "Org admins can create invitations for their organizations" 
  ON organization_invitations 
  FOR INSERT 
  WITH CHECK (
    organization_id IN (
...
CREATE POLICY "Org admins can view requests for their organizations" 
  ON organization_join_requests 
  FOR SELECT 
  USING (
    organization_id IN (
...
CREATE POLICY "Org admins can update requests for their organizations" 
  ON organization_join_requests 
  FOR UPDATE 
  USING (
    organization_id IN (
